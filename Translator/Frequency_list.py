...
from Frequency_list_functions import words_from_line, mash_em, frequency_counter, sentences_in_f, Create_xml, maketranslate
import nltk
import collections


def Frequency_list(SourceTextArray, Translation, Alignment):
    Zipped_lists = mash_em(Source_text=SourceTextArray, Translated_text=Translation, Alignment=Alignment)
     #mash_em uses the translation alignment to return zipped lists wher i[0] is the Source Text, and i[1] is the Translation.
    Emptydict = frequency_counter(Zipped_lists)
    return Emptydict

def TranslationandAlignment(SourceTextArray):
    TranslationandAlignmentlist = maketranslate(SourceTextArray)
    return TranslationandAlignmentlist
